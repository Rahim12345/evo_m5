        ...
        <div class="card-header">
            <div class="card-tools">
                <select name="language" id="language" class="form-control d-inline-block w-auto me-2"
                        onchange="window.location.href='{!! route('instructor.index') !!}?locale='+this.value">
                ...
                <a href="{{ route('instructor.create') }}?locale={{ request('locale') }}" class="btn btn-primary">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    @forelse($instructors as $instructor)
                        <tr class="instructors" data-id="{{ $instructor->id }}">
                            <td>{{ $loop->iteration }}</td>
                            <td>
                                <div class="image" style="width: 100px;height: 100px">
                                    <img src="{{ asset('files/instructors/'.$instructor->src) }}"
                                         style="object-fit: cover;height: -webkit-fill-available;"
                                         alt="{{ $instructor->alt }}">
                                </div>
                            </td>
                            <td>{{ $instructor->name }}</td>
                            <td>{{ $instructor->order_no }}</td>
                            <td>{{ $instructor->created_at->format('d-m-Y') }}</td>
                            <td>
                                <a href="{{ route('instructor.edit', $instructor->id) }}?locale={{ request('locale') }}"
...
@section('js')
    <script>
        $('#language').on('change', function () {
            window.location.href = '{{ route("instructor.index") }}?locale=' + $(this).val();
        });
